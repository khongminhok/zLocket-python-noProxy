...
# coding: utf-8
# Version: 1.0.6 (Fixed)
# Github: https://github.com/khongminhok
# OG Github: https://github.com/khongminhok
...
class zLocket:
    def __init__(self, device_token: str="", target_friend_uid: str="", num_threads: int=1, note_target: str=""):
        ...
        self.NAME_TOOL="zLocket Tool Pro - KhongminhOk"
        ...
        self.telegram='khongminhok'
        self.author='khongminh'
        self.messages=[]
        ...
        if os.name == "nt":
            os.system(
                f"title 💰 {self.NAME_TOOL} {self.VERSION_TOOL} by Khongminhok.blogspot.com 💰"
         )
    ...
    def _input_(self, prompt_text="", section="config"):
        print(
            f"{xColor.CYAN}┌──({xColor.NEON_GREEN}root@khongminhok{xColor.CYAN})-[{xColor.PURPLE}{section}{xColor.CYAN}]")
    ...
    def _zlocket_panel_(self):
        ...
        zlocket_confirm=self._input_(
            f'Nhập Y Xác Nhận Chạy Tool {xColor.RED}(LƯU Ý: {xColor.WHITE}Tool sẽ spam không dừng trừ khi bạn ấn enter hoặc đóng tool{xColor.RED}) {xColor.WHITE}[y/n]',
            "config"
        )
    ...
    def _convert_url(self, url: str) -> str:
        ...
        try:
            ...
            if _res.get("status") == 1 and "url" in _res:
                return _res["url"]
            self.messages.append("Lỗi kết nối tới API Url.TD.Com")
            return ""
        except requests.exceptions.RequestException as e:
            self.messages.append(
                "Lỗi kết nối tới API Url.TD.Com " + str(e))
            return ""
        except ValueError:
            self.messages.append("Lỗi kết nối tới API Url.TD.Com")
            return ""
...
def load_proxies():
    ...
    try:
        ...
        config._print(
            f"{xColor.YELLOW}[!] {xColor.RED}No local proxy file detected, trying currently available proxies... Fetch online proxies")
        onlProxy = requests.get('https://api.proxyscrape.com/v4/free-proxy-list/get?request=display_proxies&protocol=http&proxy_format=protocolipport&format=text&timeout=20000')
        if onlProxy.status_code == 200:
            online_proxies = onlProxy.text.splitlines()
            config._print(
                f"{xColor.MAGENTA}[+] {xColor.GREEN}Found {xColor.WHITE}{len(online_proxies)} {xColor.GREEN}proxies in online storage")
            config._loader_("Processing online proxies", 1)
            proxies.extend(online_proxies)
    if not proxies:
        config._print(
            f"{xColor.RED}[!] {xColor.YELLOW}No proxies available for operation")
...
def main():
    ...
    typing_print(
        f"-----------------[zLocket Tool Pro Cuongbok {config.VERSION_TOOL}]-----------------", delay=0.01)
    config._print(
